        ...
        let markerList = response.map( (data) => {
            ...
            kakao.maps.event.addListener( marker, 'click', function() {
                  ...
                  document.querySelector('.offcanvas-body .carousel-inner').innerHTML = carouselHTML
                    // 3. 제품 가격/내용들
                    // 4. 버튼( 찜하기 , 채팅하기 )
                    let result = plikleWrite(data.pno,'get');
                    if(result){}
                    document.querySelector('.offcanvas-body .sideBarBtnBox')
                    .innerHTML=`
                        <button onclick=plikleWrite(${data.pno},'post') type="button">찜하기 등록</button>
                        <button onclick=plikleWrite(${data.pno},'get') type="button">찜하기 취소</button>
                        <button onclick=plikleWrite(${data.pno},'delete') type="button">찜하기 삭제</button>
                    `
                    }else{
                        document.querySelector('.offcanvas-body .sideBarBtnBox')
                                        .innerHTML=`
...
function plikleWrite(pno,method){
    let result=false;
    $.ajax({
        ...
        success:(r)=>{console.log(r)
            if(!r){alert('로그인후 가능합니다');}

        }
    })
}
//3. 찜하기 상태출력함수 //1. 사이드바 열릴 때 2. 찜하기 변화가 있을 때
